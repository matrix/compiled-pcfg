...
int get_key(char *filename, char *section, char *key, char *result) {
    ...
    char *section_name = (char *) calloc (section_len + 1, sizeof (char));

    if (section_name == NULL)
    {
        fprintf (stderr, "Error: malloc(section_name) failed\n");
        fclose (config);
        return -1;
    }
    ...
    char *key_name = (char *) calloc (key_len + 1, sizeof (char));

    if (key_name == NULL)
    {
        fprintf (stderr, "Error: malloc(key_name) failed\n");
        free (section_name);
        fclose (config);
        return -1;
    }
    ...
    while (fgets(buff, MAX_CONFIG_LINE, (FILE*)config)) {
        ...
        // We are looking for the key_value
            ...
            if (strncmp(key_name, buff, key_len) == 0) {
                ...
                while (result_len > 0 && (result[result_len - 1] == '\r' || result[result_len - 1] == '\n')) result[--result_len] = '\0';
                ret = 0;
                break;
            }
        }
    ...
    // if ret is 2, Did not find the key
    free (section_name);
    free (key_name);
    fclose (config);
    return ret;
